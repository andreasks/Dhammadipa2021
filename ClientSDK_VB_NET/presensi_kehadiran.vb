...
Public Class presensi_kehadiran2
    ...
    '    '    xview2 = xDataSet.Tables("user").DefaultView
    '    '    DataGridView3.DataSource = xview2
    ...
    Private Sub isidatadownload()
        ...
        xAdapter = New MySqlDataAdapter("SELECT pin, nama, jenis_kelamin, DATE_FORMAT(tanggal_lahir, '%d/%m/%Y') AS tanggal_lahir,  wa, alamat, tlp1, tlp2, email, kategori, check_all, pujabakti, meditasi, dana_makan, baksos, fung_shen, sunskul, bursa, keakraban, pelayanan_umat, donasi, seminar, kelas_dhamma, jenis_kendaraan, no_kendaraan, tempat_lahir, goldar, nama_buddhist  FROM USER", xConnection)
        xAdapter.Fill(xDataSet, "user")
        'xDataSet.Tables("user").Rows(1).Item(4) = "Sudah Absen"
        xview2 = xDataSet.Tables("user").DefaultView
        DataGridView2.DataSource = xview2
    ...
    Private Sub isiData()
        Try
            ...
            xDataSet.Clear()
            DataGridView1.Refresh()
            xAdapter = New MySqlDataAdapter("SELECT user.pin, nama, wa, alamat, kategori, DATE_FORMAT(tanggal_lahir, '%d/%m/%Y') AS tanggal_lahir, goldar, jenis_kendaraan, no_kendaraan, IF(MAX(DATE(scan_date))=DATE(NOW()),'Hadir', 'Belum')  AS status_kehadiran FROM USER LEFT JOIN scanlog ON user.pin = scanlog.pin GROUP BY user.pin", xConnection)
            xAdapter.Fill(xDataSet, "user")
            xview2 = xDataSet.Tables("user").DefaultView
            DataGridView1.DataSource = xview2
        Catch ex As Exception
            MsgBox("Periksa Koneksi DataBase")
        End Try
    ...
    Private Sub ComboBox5_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ComboBox5.SelectedIndexChanged
        Try
            If ComboBox5.SelectedIndex = 1 Then
                xDataSet.Clear()
                DataGridView1.Refresh()
                xAdapter = New MySqlDataAdapter("SELECT user.pin, nama, wa, alamat, kategori, tanggal_lahir, goldar, jenis_kendaraan, no_kendaraan, IF(MAX(scan_date)=DATE(NOW()),'Hadir', 'Belum')  AS status_kehadiran FROM USER  LEFT JOIN scanlog ON user.pin = scanlog.pin WHERE goldar = 'A'  GROUP BY user.pin ", xConnection)
                xAdapter.Fill(xDataSet, "user")
            ...
            ElseIf ComboBox5.SelectedIndex = 4 Then
                xDataSet.Clear()
                DataGridView1.Refresh()
                xAdapter = New MySqlDataAdapter("SELECT user.pin, nama, wa, alamat, kategori, tanggal_lahir, goldar, jenis_kendaraan, no_kendaraan, IF(MAX(scan_date)=DATE(NOW()),'Hadir', 'Belum')  AS status_kehadiran FROM USER  LEFT JOIN scanlog ON user.pin = scanlog.pin WHERE goldar = 'B'  GROUP BY user.pin ", xConnection)
                xAdapter.Fill(xDataSet, "user")
            ...
            ElseIf ComboBox5.SelectedIndex = 7 Then
                xDataSet.Clear()
                DataGridView1.Refresh()
                xAdapter = New MySqlDataAdapter("SELECT user.pin, nama, wa, alamat, kategori, tanggal_lahir, goldar, jenis_kendaraan, no_kendaraan, IF(MAX(scan_date)=DATE(NOW()),'Hadir', 'Belum')  AS status_kehadiran FROM USER  LEFT JOIN scanlog ON user.pin = scanlog.pin WHERE goldar = 'O'  GROUP BY user.pin ", xConnection)
                xAdapter.Fill(xDataSet, "user")
            ...
            ElseIf ComboBox5.SelectedIndex = 10 Then
                xDataSet.Clear()
                DataGridView1.Refresh()
                xAdapter = New MySqlDataAdapter("SELECT user.pin, nama, wa, alamat, kategori, tanggal_lahir, goldar, jenis_kendaraan, no_kendaraan, IF(MAX(scan_date)=DATE(NOW()),'Hadir', 'Belum')  AS status_kehadiran FROM USER  LEFT JOIN scanlog ON user.pin = scanlog.pin WHERE goldar = 'AB'  GROUP BY user.pin ", xConnection)
                xAdapter.Fill(xDataSet, "user")
    ...
    Private Sub Button8_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button8.Click
        ...
        With ExcelSheet
            ...
            For i = 1 To Me.DataGridView1.RowCount
                .cells(i + 1, 1) = Me.DataGridView1.Rows(i - 1).Cells(0).Value
